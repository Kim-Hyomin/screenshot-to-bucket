...
- `.env`에는 API 키가 포함되므로 공개 저장소에 업로드하지 마세요.
- `data/images/`, SQLite DB, `outputs/`에는 사용자가 분석한 이미지와 모델 응답이 포함될 수 있습니다.
- 저장소의 `.gitignore`는 로컬 실행 데이터가 실수로 커밋되는 것을 줄이도록 구성되어 있습니다.
- 공개 또는 배포 전에는 스크린샷에 전화번호, 주소, 계정 정보, 알림, 결제 정보, 개인 메시지 등이 포함되어 있지 않은지 확인하세요.
- 저장소에 포함된 예제 스크린샷은 프로젝트의 비상업적 기능 시연을 위한 입력 예시입니다. 화면에 표시된 상표, 게시물, 이미지 및 기타 제3자 콘텐츠의 권리는 각 권리자에게 있습니다.
- 실제 서비스로 확장할 경우 이미지 보관 기간, 사용자 동의, 삭제 기능, API 제공자의 데이터 처리 정책을 별도로 검토해야 합니다.
